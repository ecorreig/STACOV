...
# sources
_dir = "helpers"
source(file.path(_dir, "lasso.R"), encoding = "UTF-8")
source(file.path(_dir, "corbes_roc.R"), encoding = "UTF-8")
source(file.path(_dir, "helpers.R"), encoding = "UTF-8")
source(file.path(_dir, "weights.R"), encoding = "UTF-8")
source(file.path(_dir, "grafics.R"), encoding = "UTF-8")
...
```{r, eval=F}
cmc = cuminc(dfkm$temps,
             dfkm$Mort,
             dfkm$Estatines, 
             strata = dfkm$Sexe)
ggcompetingrisks(cmc)
```
...
**Alerta:** posem el punt de tall de l'estudi a 42 dies.
